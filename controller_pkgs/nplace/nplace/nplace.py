...
class NPlaceController(Node):
    ...
    def init_variable(self):
        ...
        self.gripper_initialized = False  # 그리퍼 초기화 상태
    ...
    def init_services(self):
        ...
        self.cli_gripper_done = self.create_client(MoveDone, '/e0509/gripper_done')
    ...
    def _gripper_move_callback(self, future):
        ...
        try:
            ...
            if result and result.success:
                ...
                # 2초 후 gripper_done 서비스 호출
                threading.Timer(2.0, self._call_gripper_done).start()
            else:
                self.get_logger().error('❌ 그리퍼 이동 명령 실패')
        except Exception as e:
            self.get_logger().error(f'❌ 그리퍼 이동 오류: {str(e)}')
    
    def _call_gripper_done(self):
        """2초 후 MoveDone 서비스를 호출하여 그리퍼 동작 완료를 알림"""
        if not self.cli_gripper_done.wait_for_service(timeout_sec=1.0):
            self.get_logger().warn("gripper_done service not available")
            return
        
        req = MoveDone.Request()
        req.done = True
        
        future = self.cli_gripper_done.call_async(req)
        future.add_done_callback(self._gripper_done_response_cb)
    
    def _gripper_done_response_cb(self, future):
        """MoveDone 서비스 응답 콜백"""
        try:
            response = future.result()
            if response.success:
                self.get_logger().info(f"✅ Gripper done signal sent: {response.message}")
            else:
                self.get_logger().warn(f"⚠️ Gripper done signal failed: {response.message}")
        except Exception as e:
            self.get_logger().error(f"Gripper done service call failed: {e}")
