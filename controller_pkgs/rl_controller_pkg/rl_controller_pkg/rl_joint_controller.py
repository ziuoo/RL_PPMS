...
class PPOPositionControlNode(Node):
    def __init__(self):
        ...
        # MoveStop 서비스 클라이언트 생성 (모션 큐 비우기용)
    ...
    def _call_move_stop(self):
        """
        MoveStop 서비스를 호출하여 모션 큐 비우기
        DR_SSTO(2) = Soft Stop
        """
        if not self.move_stop_client.wait_for_service(timeout_sec=0.5):
            self.get_logger().warn("[MoveStop] Service not available")
            return
        
        request = MoveStop.Request()
        request.stop_mode = 2  # DR_SSTO = Soft Stop
        
        future = self.move_stop_client.call_async(request)
        future.add_done_callback(self._move_stop_response_cb)
    
    def _move_stop_response_cb(self, future):
        """
        MoveStop 서비스 응답 콜백
        """
        try:
            response = future.result()
            if response.success:
                self.get_logger().info("🛑 [MoveStop] Motion queue cleared successfully")
            else:
                self.get_logger().warn("⚠️ [MoveStop] Failed to clear motion queue")
        except Exception as e:
            self.get_logger().error(f"[MoveStop] Service call exception: {e}")
    ...
    def compute_loop(self):
        ...
        try:
            ...
            if dist < self.stop_distance_threshold:
                ...
                # 30초 대기 후 MoveDone 서비스 호출
                self.get_logger().info("⏳ Waiting 30 seconds before sending MoveDone signal...")
                
                def delayed_done():
                    import time
                    time.sleep(1)
                    self.get_logger().info("✅ 1 seconds elapsed. Sending MoveDone signal.")
