...
class CommandsCfg:
    ...
    object_pose:
      - 로봇 base_link 기준 EE가 도달해야 하는 목표 pose (x,y,z,qw,qx,qy,qz)
      - HybridPoseCommandCfg: 위치는 범위에서 샘플링, 자세는 미리 정의한 orientation 중에서 선택
    """

    object_pose = mdp.HybridPoseCommandCfg(
        asset_name="robot",
        body_name=MISSING,  # will be set by agent env cfg (예: EE 링크 이름)
        ...
        predefined_orientations=[
            # Top-down grasp 스타일
            quat_from_euler_xyz(
                roll=torch.tensor([0.0]), #1.57
                pitch=torch.tensor([3.14]),
                yaw=torch.tensor([0.0]) #-3.14
            ).squeeze().tolist(),
        ],
    )
...
class ActionsCfg:
    ...
    arm_action: mdp.JointPositionActionCfg | mdp.RelativeJointPositionActionCfg | mdp.DifferentialInverseKinematicsActionCfg = MISSING
...
class ObservationsCfg:
    ...
    class PolicyCfg(ObsGroup):
        ...
        # Target pose command (x,y,z,qw,qx,qy,qz) in base frame
        target_pose = ObsTerm(
            func=mdp.generated_commands,
            params={"command_name": "object_pose"},
        )

        # Previous action - arm only (delta joint action)
...
class RewardsCfg:
    ...
    # EE position → 목표 포지션까지 가는 reward (exp 커널)
    ...
    # EE orientation → 3축 정렬 multi-axis alignment
    ...
    # action penalty (delta joint 크기 줄이기)
    action_rate = RewTerm(func=mdp.action_rate_l2, weight=-1e-4)

    # joint velocity penalty (속도 너무 크면 감점)
    joint_vel = RewTerm(
...
class TerminationsCfg:
    ...
    # 2) EE가 목표 포즈(위치+자세)에 충분히 도달하면 종료
    goal_reached = DoneTerm(
        func=mdp.ee_reached_command_pose,
        params={
            "command_name": "object_pose",
            "pos_threshold": 0.01,   # 1 cm
            "rot_threshold": 0.15,   # ~8.6 deg
            "ee_frame_cfg": SceneEntityCfg("ee_frame"),
        },
    )

    # 3) medicine cabinet과 너무 가까워지면 종료 (충돌 방지)
